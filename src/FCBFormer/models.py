...
from src.FCBFormer.pvt_v2 import PyramidVisionTransformerV2
...
class TB(nn.Module):
    def __init__(self, model_dir='/root/.cache/huggingface/forget/pvt_v2_b2.pth'):
...
class FCBFormer(nn.Module):
    def __init__(self, size=352, num_class=1, model_dir='/root/.cache/huggingface/forget/pvt_v2_b2.pth'):

        super().__init__()

        self.TB = TB(model_dir)
        self.FCB = FCB(in_resolution=size)
        self.PH = nn.Sequential(
            RB(64 + 32, 64), RB(64, 64), nn.Conv2d(64, num_class, kernel_size=1)
        )
...
if __name__ == '__main__':
    ...
    model = FCBFormer(size=352).to(device)
