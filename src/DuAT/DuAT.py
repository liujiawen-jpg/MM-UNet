...
from src.DuAT.pvtv2 import pvt_v2_b2
...
class SBA(nn.Module):
    def __init__(self,input_dim = 64, out_channels=1):
        super().__init__()
        ...
        self.conv = nn.Sequential(BasicConv2d(input_dim, input_dim, 3,1,1), nn.Conv2d(input_dim, out_channels, kernel_size=1, bias=False))
...
class DuAT(nn.Module):
    def __init__(self,in_channels=3, out_channels=1, dim=32, dims= [64, 128, 320, 512], model_dir = '/workspace/Encvis/Code/src/DuAT/pvt_v2_b2.pth'):
        super(DuAT, self).__init__()
        self.backbone = pvt_v2_b2(in_chans=in_channels,embed_dims=dims)  # [64, 128, 320, 512]
        ...
        self.SBA = SBA(input_dim = dim,out_channels=out_channels)
        self.fuse = BasicConv2d(dim * 2, dim, 1)
        self.fuse2 = nn.Sequential(BasicConv2d(dim*3, dim, 1,1),nn.Conv2d(dim, out_channels, kernel_size=1, bias=False))
...
if __name__ == '__main__':
    ...
    model = DuAT(in_channels=3,out_channels=2).to('cuda')
