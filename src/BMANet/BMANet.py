...
class BMANet(nn.Module):
    def __init__(self, channel=64, out_channel=1, model_dir = '/workspace/Encs/src/CVCUNETR/pvt_v2_b2.pth'):
        ...
        self.fuse = BasicConv2d(1, out_channel, 1)
    ...
    def forward(self, x):
        ...
        # main_x = torch.cat([self.upsample(main_out,x), self.upsample(edge,x), self.upsample(global_map,x), self.upsample(side_out4,x), self.upsample(side_out3,x)], dim=1)
        out = self.fuse(main_out)
        out = self.upsample(out,x)
        return out

if __name__ == '__main__':
    device = 'cuda:1'
    model = BMANet(channel=64, out_channel=5, model_dir = '/workspace/Encs/src/CVCUNETR/pvt_v2_b2.pth').to(device)
