...
def generate_PolypG_dataset_list(dataset_root: str):
    dataset_list = []
    train_folders = os.listdir(dataset_root)
    for folder in train_folders:
        image_path = os.path.join(dataset_root, folder, "images")
        label_path = os.path.join(dataset_root, folder, "masks")
        for image in os.listdir(image_path):
            image_name = image.split(".")[0]
            dataset_list.append(
            {
                "image": os.path.join(image_path, image),
                "label": os.path.join(label_path, image_name + "_mask.jpg"),
            }
        )
    return dataset_list


class PolypGDataset(data.Dataset):
    def __init__(
        self,
        samples: list,
...
def get_dataloader(config):
    data_root = config.dataset.PolypGen.data_root
    train_ratio = config.dataset.PolypGen.train_ratio
    dataset_list = generate_PolypG_dataset_list(data_root)
    ...
    (
        ...
        image_size=config.dataset.PolypGen.image_size, image_mean=config.dataset.PolypGen.image_mean, image_std=config.dataset.PolypGen.image_std
    )
    train_dataset = PolypGDataset(
        train_images, train_transform_image, train_transform_label
    )
    val_dataset = PolypGDataset(val_images, val_transform_image, val_transform_label)
    train_loader = data.DataLoader(
        dataset=train_dataset,
        batch_size=config.dataset.PolypGen.batch_size,
        shuffle=True,
        num_workers=config.dataset.PolypGen.num_workers,
        pin_memory=True,
    )
